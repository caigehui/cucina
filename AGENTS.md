...
研报默认按 `America/New_York` 时区在美股常规交易日 07:30 与 18:00 两次推送,分别对应常规开盘 09:30 前 2 小时和常规收盘 16:00 后 2 小时。夏令时/冬令时由美东时区自动换算,不固定写死北京时间。若用户要求创建、查看、修改、删除定时任务,必须先搜索并使用 automation/update 类工具,不要手写自动化指令。
