...
1. 使用长桥 Longbridge MCP/官方技能读取只读数据,在用户明确要求时分析美股持仓。
2. 定时生成《每日美股研报》,只覆盖公开市场、宏观事件、社媒情绪和科技/AI 供应链,不读取、不分析、不输出账户或持仓内容。
3. 对单只美股做技术分析,包括 K 线、趋势、期权结构、资金和图表。
4. 给出投资建议,但必须表现为情景预案、触发条件和风险说明,不能表现为无条件交易指令。
5. 将公开或脱敏材料整理为 MiroFish seed 包,把独立 MiroFish 生成的报告作为情景仿真旁路输入。
...
- MiroFish 输出只能标为模拟推断/推测,不能写成行情事实、价格目标或无条件交易指令。
...
每日研报属于公开市场研究,不是持仓分析。即使自动化提示或上游任务写了“只读持仓提示”“账户风险”“持仓风险提示”,也必须忽略这类要求;不得调用账户/持仓接口,不得生成“只读账户与持仓风险提示”或任何等价 section。只有用户明确说“分析我的持仓”“看看我的仓位”“给我持仓预案”时,才走 `portfolio-advisor`。

每日研报和个股技术分析正文落盘前,必须用 `stop-ai-slop-zh` 做一次中文表达体检:删套话、拆排比、去名词化、减少抽象主语。润色只改表达,不得删来源、as-of 时间、关键数字、图表引用、风险声明和事实/传闻/推断/推测分层。

## 长桥 Longbridge 使用规则

- 长桥相关能力优先用于只读数据:行情、K 线、盘口、期权链、资金流、新闻、财报、评级、日历。账户和持仓数据只允许在用户明确要求持仓/账户分析时读取,不得用于每日研报。
...
每日研报禁止包含账户、现金、持仓、仓位、组合、券商账户风险或“只读持仓提示”内容。`output/reports/` 下的日报不得出现“只读账户与持仓风险提示”或任何同义 section;主要来源也不得列账户/持仓接口。
...
- 只使用公开材料、已验证研究摘要或脱敏组合暴露。
- 不默认写入精确账户资产、成本、持仓数量、订单、流水、API Key、token、cookie 或私钥。
- MiroFish 独立运行;不要把 MiroFish 源码复制进本仓库。
- 输出进入 Cucina 时必须分层为已证实事实、模拟推断、推测假设、触发条件和证伪条件。
- 涉及仓位动作时必须写成情景预案,并保留风险声明。
